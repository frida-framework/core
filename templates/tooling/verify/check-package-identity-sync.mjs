...
const CONTRACT_IDENTITY_PATH = path.join(ROOT_DIR, 'contract', 'layers', 'FL01-identity.yaml');
...
if (contractIdentity?.CONTRACT_META?.scope?.package !== expectedPackageName) {
  failures.push(
    `contract/layers/FL01-identity.yaml package mismatch (${contractIdentity?.CONTRACT_META?.scope?.package} !== ${expectedPackageName})`,
  );
}
