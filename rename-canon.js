...
for (const file of files) {
    ...
    if (file.includes('rename-canon.js')) continue;
    ...
    // Replace CANON -> CONTRACT
    content = content.replace(/CANON/g, 'CONTRACT');
    // Replace canon -> contract (except inside specific URLs if necessary, but we own schemas.frida.dev/frida-contract.schema.json now)
    content = content.replace(/canon/g, 'contract');
...
console.log(`Updated content in ${modifiedFiles} files.`);
