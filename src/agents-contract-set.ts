...
function getZoneEntries(
  ...
  return Object.entries(contract.ZONES)
    ...
    .filter(([, value]) => options.includeFridaInternal || !isEngineOwnedZone(contract, value));
}
...
function collectExpectedAgents(
  ...
  for (const [zoneId, zoneData] of getZoneEntries(contract, options)) {
...
export function checkAgentsContractSet(options: AgentsContractSetCheckOptions = {}): AgentsContractSetCheckResult {
  ...
  const expectedSet = collectExpectedAgents(contract, { includeFridaInternal });
  const actualSet = collectTrackedAgents(rootDir, { includeFridaInternal });
  const missing = toSortedDiff(expectedSet, actualSet);
  const extra = toSortedDiff(actualSet, expectedSet);
...
export function runFridaAgentsContractSetCheck(options: AgentsContractSetCheckOptions = {}): number {
  try {
    ...
    if (!result.ok) {
      ...
      console.error(`Expected tracked AGENTS.md files: ${result.expected.length}`);
      console.error(`Actual tracked AGENTS.md files:   ${result.actual.length}\n`);
    ...
    console.log(`AGENTS contract-set OK (${result.actual.length} tracked file(s))`);
    return 0;
  } catch (error) {
